...
def get_tags():
    ...
    global glove, nlp, fast_text, word2vec, keybert
    if not glove:
    ...
    try:
        ...
        kb_tags = keybert.extract_keywords(text, keyphrase_ngram_range=(1, 3))
        keybert_tags = {text2camel(tag[0]): float(tag[1]) for tag in kb_tags}
        kb_tags = keybert.extract_keywords(text, keyphrase_ngram_range=(1, 2))
        keybert_tags.update({text2camel(tag[0]): float(tag[1]) for tag in kb_tags})
        kb_tags = keybert.extract_keywords(text, keyphrase_ngram_range=(1, 1))
        keybert_tags.update({text2camel(tag[0]): float(tag[1]) for tag in kb_tags})
    except Exception as e:
        logger.exception(e)
    result['keybert_tags'] = keybert_tags

    # Merge tags from above 4 methods
    tags = list(spacy_hotwords.keys()) + list(rake_spacy_tags.keys()) + list(rake_nltk_tags.keys()) + list(keybert_tags.keys())
